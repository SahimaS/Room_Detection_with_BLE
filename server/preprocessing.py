...
rf_accuracy = 96.67  # Hardcoded from your provided accuracy
knn_accuracy = 90.0  # Hardcoded from your provided accuracy
...
def collect_test_data(data):
    ...
    if len(result_A) >= 1 and len(result_B) >= 1 and len(result_C) >= 1:
        feature_data = [result_A[0], result_B[0], result_C[0]]
        feature_data_array = np.array(feature_data).reshape(1, -1)
        feature_data_array = feature_data_array.astype(np.float64)
